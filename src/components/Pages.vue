<template>
  <div>
    <div class="background-content" v-for="(value, key, index) in this.data.arr" v-if="value.url === $route.params.id">
        <section class="offset60">
            <h1 contenteditable="false" id="title">{{value.title}}</h1>
        </section>
        <section class="offset0">
            <div id="image" class="imgHero" v-bind:style="{ backgroundImage: 'url(' + display(value.img) + ')' }"></div>
        </section>
  ...
  methods: {
    ...
    getUrl (id) {
      ...
      for (var i = this.data.arr.length - 1; i >= 0; i--) {
        if (Number(id) === Number(this.data.arr[i].id)) {
