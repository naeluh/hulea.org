<template>
  <div>
    <div class="background-content" v-for="a in this.data.arr" v-if="a.id === $route.params.id">
        <section class="offset60">
            <h1 contenteditable="false" id="title">{{a.title}}</h1>
        </section>
        <section class="offset0">
            <div id="image" class="imgHero" v-bind:style="{ backgroundImage: 'url(' + display(a.img) + ')' }"></div>
        </section>
        <section class="offset60_2">
            <h3><a contenteditable="false" id="link" target="_blank" href="http://gifpaint.hulea.org/"></a></h3>
            <p id="description" contenteditable="false" v-html="a.siteInfo"></p>
        </section>
        <section class="offset40">
            <a id="next" class="next" href=""><i class="fa fa-arrow-right" aria-hidden="true"><icon name="arrow-right" scale="3"></icon></i></a>
        </section>
    </div>
  </div>
</template>
